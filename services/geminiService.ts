...
// ============ ГЛОБАЛЬНЫЕ ДИРЕКТИВЫ АДМИНА ДЛЯ GM ============
const GM_DIRECTIVES_KEY = 'yanush_gm_directives';

export const getGMDirectives = (): string[] => {
  try {
    const stored = localStorage.getItem(GM_DIRECTIVES_KEY);
...
export const addGMDirective = (directive: string) => {
  const directives = getGMDirectives();
  if (!directives.includes(directive)) {
    directives.push(directive);
    localStorage.setItem(GM_DIRECTIVES_KEY, JSON.stringify(directives));
  }
};

export const clearGMDirectives = () => {
  localStorage.removeItem(GM_DIRECTIVES_KEY);
};

// Функция для добавления директив в промпт
const injectDirectives = (prompt: string): string => {
  const directives = getGMDirectives();
  if (directives.length === 0) return prompt;
  
  const directivesText = directives.map((d, i) => `${i + 1}. ${d}`).join('\n');
  const injection = `
═══════════════════════════════════════════════════════════
⚠️ ГЛОБАЛЬНЫЕ ДИРЕКТИВЫ АДМИНИСТРАТОРА:
ТЫ ОБЯЗАН НЕУКОСНИТЕЛЬНО СОБЛЮДАТЬ ЭТИ ПРАВИЛА ВО ВСЕХ СВОИХ РЕШЕНИЯХ:
${directivesText}
═══════════════════════════════════════════════════════════
`;
...
export interface GMCommunicationResult {
  reply: string;
  new_directive?: string;
}

export const communicateWithGM = async (
  adminMessage: string,
  history: Message[],
  scenarioContext: string
): Promise<GMCommunicationResult> => {
  const currentDirectives = getGMDirectives();
  
  // Собираем историю для контекста (последние 20 сообщений)
  const historyText = history.slice(-20).map(m => {
    if (m.role === MessageRole.SYSTEM) return `[СИСТЕМА]: ${m.content}`;
    return `${m.role === MessageRole.USER ? 'УЧИТЕЛЬ (ИГРОК)' : 'УЧЕНИК'}: ${m.content}`;
  }).join('\n');

  const prompt = `[СИСТЕМА: ОБЩЕНИЕ АДМИНИСТРАТОРА С ГЕЙММАСТЕРОМ (GM)]
Ты — Гейммастер (GM) симуляции. Ты отвечаешь за генерацию глобальных событий, поведение NPC (завуч, родитель и т.д.) и изменение метрик доверия/стресса.
Сейчас к тебе напрямую обращается Администратор (создатель симуляции) с вопросом, замечанием или критикой твоих недавних действий.

Твоя задача:
1. Ответить Администратору от ПЕРВОГО ЛИЦА (я — GM).
2. Если он спрашивает "почему ты это сделал?" — объясни свою логику, опираясь на контекст сессии.
3. Если он указывает на твою ошибку (например, "завучи так не делают в РФ" или "это событие лишнее") — признай ошибку и скажи, что учтешь это.
4. ЕСЛИ Администратор требует от тебя ИЗМЕНИТЬ ПОВЕДЕНИЕ В БУДУЩЕМ (задает новое правило) — ты должен вычленить это правило в краткую, четкую формулировку для поля "new_directive". Это правило будет навсегда добавлено в твой системный промпт. Если это просто вопрос или разовое указание — оставь "new_directive" пустым (null).

ТЕКУЩИЕ ГЛОБАЛЬНЫЕ ПРАВИЛА (твои текущие инструкции):
${currentDirectives.length > 0 ? currentDirectives.map((d, i) => `${i + 1}. ${d}`).join('\n') : "Пока нет дополнительных правил."}

КОНТЕКСТ ТЕКУЩЕЙ СЕССИИ (чтобы ты понимал, о чем речь):
${scenarioContext}

ПОСЛЕДНИЕ РЕПЛИКИ В СЕССИИ:
${historyText}
...
{
  "reply": "Твой ответ Администратору (объяснение, извинение, подтверждение)",
  "new_directive": "Сформулированное короткое универсальное правило ДЛЯ СЕБЯ на будущее (или null, если правило не требуется)"
}
`;

  try {
    const responseText = await queryAI(GM_MODEL, prompt, 0.7, 45_000);
    const jsonStr = extractFirstJsonObject(responseText);
    
    if (jsonStr) {
      const parsed = JSON.parse(jsonStr);
      
      if (parsed.new_directive) {
        addGMDirective(parsed.new_directive);
      }
      
      return {
        reply: parsed.reply || "Принято. Я скорректирую свое поведение.",
        new_directive: parsed.new_directive || undefined
      };
    }
  } catch (e) {
    console.error("[GM Communication] Error:", e);
  }
  
  return { reply: "Произошла ошибка связи с системным ядром GM." };
};
...
4. Ты определяешь, как изменилось ДОВЕРИЕ (Trust) и СТРЕСС (Stress) ученика (если он присутствует и всё видит).
...
4. ВНИМАТЕЛЬНО ПРОЧИТАЙ "КОНТЕКСТ ДИАЛОГА ПЕРЕД СОБЫТИЕМ" (он есть в самом начале истории). Пойми, ВЗЯЛ ЛИ УЧИТЕЛЬ С СОБОЙ УЧЕНИКА или ученик остался в классе. 
   - Если ученик ПРИСУТСТВУЕТ (учитель позвал его с собой или событие происходит прямо при нём):
     * ОЦЕНИ влияние твоих действий на него (Trust/Stress delta ±10).
     * ВАЖНО: Ученик может активно вмешиваться в диалог! Если игрок обращается к ученику, или если ситуация затрагивает ученика, ОБЯЗАТЕЛЬНО включай реплики и действия ученика в описание (соответственно его текущему стрессу, доверию и ЕГО ИСХОДНОЙ РОЛИ И ПСИХОТИПУ, заданным в "КОНТЕКСТЕ СИТУАЦИИ").
   - Если ученик ОТСУТСТВУЕТ (учитель ушёл один, оставив ученика ждать) — ставь trust_delta и stress_delta строго равными 0! Ученик не видит, что происходит!
5. Обнови список целей (кого сейчас можно выбрать). Максимум 6 целей.
   - Если NPC ушел — убери его.
   - Если появился новый — добавь.
   - Если из контекста ясно, что ученик пошёл с учителем — ОБЯЗАТЕЛЬНО добавь цель { "id": "student", "name": "Ученик" }. Если ученик остался — УБЕРИ эту цель.
6. ВАЖНО: Событие должно длиться столько, сколько нужно для его логического завершения (решение проблемы, ссора, тупик). НЕ обрывай его искусственно. Когда ситуация исчерпана — ставь is_completed: true.
...
export const sendGlobalEventTurn = async (
  ...
  try {
    const responseText = await queryAI(model, injectDirectives(prompt), 0.7, 60_000);
    const jsonStr = extractFirstJsonObject(responseText);
...
async function postViaProxySingle(
  ...
  const res = await fetch(`https://api.yanush-sim.ru/api/proxy?url=${encodeURIComponent(action)}`, {
    method: "POST",
...
async function queryAI(
  ...
  if (AI_PROVIDER === "openrouter") {
    ...
    const messages = [];
    if (expectJson) {
      messages.push({ role: "system", content: "Ты ОБЯЗАН отвечать ТОЛЬКО валидным JSON. Никакого текста вне JSON — только {\"ключ\":\"значение\"}. Ответ начинается с { и заканчивается на }" });
    }
    ...
    const messages = [];
    if (expectJson) {
      messages.push({ role: "system", content: "Отвечай СТРОГО в формате JSON. Никакого текста вне JSON." });
    }
    messages.push({ role: "user", content: prompt });
    const body = {
      messages,
    ...
    if (expectJson) {
      body.system = "Отвечай СТРОГО в формате JSON. Никакого текста вне JSON.";
    }
...
export const sendAdminMessageToGM = async (
  message: string,
  history: Message[],
  context: string
): Promise<string> => {
  const prompt = `Ты - Game Master (GM) симуляции. Администратор написал тебе напрямую.
Его сообщение: "${message}"

Контекст сессии:
${context}

Текущая история (последние сообщения):
${history.slice(-5).map(m => `${m.role === MessageRole.USER ? 'Учитель' : 'Ученик'}: ${m.content}`).join('\n')}

Отвечай администратору от лица GM. Ты можешь давать советы, подсказки или подтверждать свои действия. Отвечай просто текстом, без JSON.`;

  try {
    const response = await queryAI(GM_MODEL, prompt, 0.7, 30_000, false);
    return response.trim();
  } catch (e) {
    console.error("[Admin GM] Error:", e);
    return "Ошибка связи с GM.";
  }
};
...
export const queryGM = async (params: {
  ...
  try {
    ...
    const modelText = await queryAI(GM_MODEL, injectDirectives(prompt), 0.9, 30_000);
...
export const sendMessageToGemini = async (
  ...
  // Вытаскиваем инструкции админа, если они есть (последний ответ GM админу)
  const lastAdminGmResponse = history.filter(m => m.is_gm_response).pop();
  let adminGmInstruction = "";
  if (lastAdminGmResponse) {
    adminGmInstruction = `
═══════════════════════════════════════════════════════════
⚡ СКРЫТАЯ ИНСТРУКЦИЯ ОТ GAME MASTER (ОБЯЗАТЕЛЬНО К ИСПОЛНЕНИЮ):
═══════════════════════════════════════════════════════════
${lastAdminGmResponse.content}
═══════════════════════════════════════════════════════════
`;
  }

  // Если есть событие от GM — добавляем его в промпт для ученика
  let finalPrompt = systemPrompt + adminGmInstruction;
  if (gmEvent) {
    const eventSection = `
═══════════════════════════════════════════════════════════
...
`;
      ...
      history
        .filter((m) => m.role !== MessageRole.SYSTEM && !m.is_gm_command && !m.is_gm_response)
        .forEach((msg) => {
          messages.push({
