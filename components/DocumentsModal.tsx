  ...
  },
  {
    ...
    content: (
      <div className="space-y-4 text-slate-400 text-sm leading-relaxed whitespace-pre-wrap">
        ...
        <p><strong>1.10. Политика конфиденциальности</strong> — отдельный документ, определяющий порядок сбора, обработки, хранения и защиты персональных данных Лицензиата, доступный в разделе <button onClick={() => setActiveDocId('privacy')} className="text-blue-400 hover:underline">«Политика конфиденциальности»</button> данного окна.</p>
        ...
        б) ознакомился с настоящей Офертой, <button onClick={() => setActiveDocId('privacy')} className="text-blue-400 hover:underline">Политикой конфиденциальности</button>, описанием Тарифов и принимает их в полном объёме;<br/>
        в) даёт согласие на обработку своих персональных данных в объёме и порядке, определённых <button onClick={() => setActiveDocId('privacy')} className="text-blue-400 hover:underline">Политикой конфиденциальности</button>;<br/>
        ...
        в) самостоятельно отслеживать изменения настоящей Оферты и <button onClick={() => setActiveDocId('privacy')} className="text-blue-400 hover:underline">Политики конфиденциальности</button>;<br/>
        ...
        <p>9.1. Совершая Акцепт, Лицензиат даёт своё информированное и добровольное согласие на обработку персональных данных в объёме, целях, способами и в сроки, определённые <button onClick={() => setActiveDocId('privacy')} className="text-blue-400 hover:underline">Политикой конфиденциальности</button>.</p>
        <p>9.2. <button onClick={() => setActiveDocId('privacy')} className="text-blue-400 hover:underline">Политика конфиденциальности</button> является неотъемлемой частью настоящей Оферты и содержит:<br/>
        ...
        <p>15.2. Полнота соглашения. Настоящая Оферта (совместно с <button onClick={() => setActiveDocId('privacy')} className="text-blue-400 hover:underline">Политикой конфиденциальности</button> и описанием Тарифов) представляет собой полное соглашение между Лицензиаром и Лицензиатом относительно предмета Договора и заменяет все предшествующие устные и письменные договорённости.</p>
  ...
  },
  {
    ...
    content: (
      <div className="space-y-4 text-slate-400 text-sm leading-relaxed whitespace-pre-wrap pb-10">
        ...
        <p>1.4. Настоящая Политика является неотъемлемой частью <button onClick={() => setActiveDocId('terms')} className="text-blue-400 hover:underline">Лицензионного договора-оферты</button> о предоставлении права использования программного комплекса «ЯНУШ» (далее — Оферта), размещенной по адресу: https://yanush-sim.ru. Термины, не определенные в настоящей Политике, используются в значениях, установленных Офертой.</p>
        ...
        <p>2.7. Пользователь (Субъект персональных данных) — любое физическое лицо, посещающее Сайт и/или зарегистрированное на Платформе.</p>
        <p>2.8. Сайт (Платформа) — совокупность графических и информационных материалов, программ для ЭВМ и баз данных, обеспечивающих их доступность в сети Интернет по адресу: https://yanush-sim.ru.</p>
        ...
        Файлы cookie и аналогичные технологии: Подробности — в разделе 8 настоящей Политики.</p>
        ...
        Исполнение <button onClick={() => setActiveDocId('terms')} className="text-blue-400 hover:underline">Лицензионного договора-оферты</button>: регистрация, создание и ведение Личного кабинета, аутентификация, предоставление доступа к функционалу Платформы, начисление и списание Сессий, обработка оплаты. (Основание: ч. 1 п. 5 ст. 6 152-ФЗ).<br/>
        Генерация контента Сессий: обработка текстовых реплик внутри Платформы для формирования запросов к Провайдерам LLM API. Передача реплик Провайдерам LLM API осуществляется исключительно в полностью обезличенном виде, без передачи каких-либо идентификаторов (email, UUID, IP-адрес и т.д.), позволяющих прямо или косвенно определить Пользователя. В связи с этим данная передача не является передачей персональных данных, в том числе трансграничной. (Основание: ч. 1 п. 5 ст. 6 и ч. 1 п. 1 ст. 6 152-ФЗ).<br/>
        ...
        <p>6.1. Для достижения целей обработки Оператор может передавать персональные данные следующим категориям получателей:<br/>
        ...
        <p>6.4. Взаимодействие с Провайдерами LLM API (в том числе иностранными сервисами машинного обучения). Для генерации ответов в симуляциях Оператор использует API сторонних сервисов. Оператор передает таким сервисам исключительно тексты реплик в полном отрыве от любых персональных данных Пользователя. Риск случайной передачи персональных данных в текстах реплик минимизирован запретом, установленным в п. 4.3 настоящей Политики. Получаемая Провайдерами LLM API информация не позволяет идентифицировать Пользователя и не является персональными данными, поэтому такая передача не подпадает под регулирование о передаче персональных данных.</p>

        <h3 className="text-lg font-semibold text-white mt-6 mb-2">7. ПРАВА СУБЪЕКТА ПЕРСОНАЛЬНЫХ ДАННЫХ</h3>
        <p>7.1. Пользователь имеет право на: информацию об обработке своих ПДн; доступ к данным; уточнение, блокирование или удаление данных; отзыв согласия; обжалование действий Оператора в Роскомнадзоре или суде.</p>
        <p>7.2. Порядок реализации прав. Запрос направляется на адрес: support@yanush-sim.ru. В запросе указываются: ФИО (при наличии), email регистрации, существо требования.</p>
        <p>7.3. Срок рассмотрения. Оператор направляет ответ в течение 30 (тридцати) календарных дней.</p>
        <p>7.4. Последствия отзыва согласия. Отзыв согласия влечет прекращение обработки ПДн, удаление учетной записи в течение 30 дней и невозможность использования Платформы.</p>
        <p>7.5. Контакт Роскомнадзора: г. Москва, Китайгородский проезд, д. 7, стр. 2; Портал обращений: https://pd.rkn.gov.ru.</p>

        <h3 className="text-lg font-semibold text-white mt-6 mb-2">8. ФАЙЛЫ COOKIE И АНАЛОГИЧНЫЕ ТЕХНОЛОГИИ</h3>
        <p>8.1. Платформа использует файлы cookie для обеспечения функционирования Сайта (строго необходимые cookie) и сбора аналитических данных (аналитические cookie Яндекс.Метрики).</p>
        <p>8.2. Пользователь может управлять файлами cookie через настройки браузера или баннер на Сайте. Отключение строго необходимых cookie приведет к невозможности использования Платформы.</p>

        <h3 className="text-lg font-semibold text-white mt-6 mb-2">9. СРОКИ ОБРАБОТКИ И ПОРЯДОК УДАЛЕНИЯ ПЕРСОНАЛЬНЫХ ДАННЫХ</h3>
        <p>9.1. Персональные данные хранятся в течение следующих сроков:<br/>
        ...
        <p>9.2. Удаление аккаунта. При удалении учетной записи данные уничтожаются (либо изолируются на срок, требуемый налоговым законодательством). Обезличенные агрегированные данные сохраняются.</p>

        <h3 className="text-lg font-semibold text-white mt-6 mb-2">10. МЕРЫ ПО ОБЕСПЕЧЕНИЮ БЕЗОПАСНОСТИ ПЕРСОНАЛЬНЫХ ДАННЫХ</h3>
        <p>10.1. Оператор принимает правовые, организационные и технические меры для защиты персональных данных.</p>
        <p>10.2. Реагирование на инциденты (утечки). В случае выявления инцидента безопасности Оператор обязуется уведомить Роскомнадзор в течение 24 часов и затронутых Пользователей не позднее 72 часов с описанием принятых мер (в порядке ч. 3.1 ст. 21 152-ФЗ).</p>

        <h3 className="text-lg font-semibold text-white mt-6 mb-2">11. ПОРЯДОК ВНЕСЕНИЯ ИЗМЕНЕНИЙ В ПОЛИТИКУ</h3>
        <p>11.1. Оператор вправе вносить изменения в настоящую Политику. Новая редакция вступает в силу по истечении 10 (десяти) календарных дней с момента ее размещения по адресу: https://yanush-sim.ru</p>
        <p>11.2. Оператор уведомляет Пользователей о существенных изменениях по электронной почте не позднее дня размещения новой редакции.</p>

        <h3 className="text-lg font-semibold text-white mt-6 mb-2">12. ЗАКЛЮЧИТЕЛЬНЫЕ ПОЛОЖЕНИЯ</h3>
        <p>12.1. Пользователь может получить любые разъяснения по вопросам обработки его персональных данных по email: support@yanush-sim.ru.</p>
        <p>12.2. Настоящая Политика действует бессрочно до ее замены новой редакцией.</p>
        <p>12.3. В случае противоречия между настоящей Политикой и <button onClick={() => setActiveDocId('terms')} className="text-blue-400 hover:underline">Офертой</button> в вопросах обработки персональных данных приоритет имеет настоящая Политика как специальный документ.</p>
      </div>
    )
  },
  {
